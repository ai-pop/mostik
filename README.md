...
Мостик — для нашего сообщества. При первом запуске откроется окно входа: нажмите
**«Войти через Discord»**, разрешите приложение в браузере — и всё, дальше программа пускает вас
без вопросов и сама обновляет проверку раз в неделю.

* Пароли, почту и номера телефонов вводить не нужно: вход идёт через официальную страницу Discord.
* Если вы ещё не в сообществе — на том же экране есть кнопка **«Вступить в сервер»**.
* Нет интернета? Программа продолжит работать до 7 дней с момента последнего входа.
...
Ключ API создаётся прямо на вашем компьютере при первом запуске — придумывать ничего не нужно.
...
Запросы к `/v1/*` дополнительно требуют действующий пропуск, который выдаётся при входе через
Discord в окне «Мостик» (проверка — раз в неделю, оффлайн-запас — 7 дней). Без пропуска сервис
отвечает `403` с пояснением. Пропуск привязан к вашему компьютеру и хранится только на нём.
...
| `Gate.Enabled` | `true` | Проверка входа: только участники сообщества |
| `Gate.GraceDays` | `7` | Сколько дней работа разрешена без связи с Discord |
...
| программа просит войти | нажмите «Войти через Discord»; если вы не в сообществе — кнопка «Вступить в сервер» |
| «сессия недействительна» | вход устарел или доступ отозван — войдите заново |
...
* Вход выполняется на официальной странице Discord. Приложение не видит ваш пароль и не может
  писать сообщения от вашего имени — оно получает только сведения об участии в сообществе.
...
2. Run `Запустить Мостик.cmd` (Start Mostik), then sign in with Discord — Mostik is available to
   members of our community (the join link is on the sign-in screen).
